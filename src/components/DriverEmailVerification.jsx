...
const DriverEmailVerification = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://www.leosdrive.com/api/driver_email_verification.php",
        { otp },
      ...
      if (response.data.success) {
        Swal.fire({ icon: "success", title: "Verified!", text: response.data.message }).then(() => {
          navigate("/driverlogin"); // Redirect to the dashboard after verification
        });
      } else {
  ...
  return (
    <Container>
      <Title>Driver Email Verification</Title>
      <FormWrapper>
        ...
        <TextLink onClick={() => navigate("/driverresendemailotp")}>Resend OTP</TextLink>
      </FormWrapper>
    </Container>
  );
};
